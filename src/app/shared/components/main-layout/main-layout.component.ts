...
import {
  CartFavoritesService
}

  from '../../service/cart-favorites.service';

import {
  SearchProductService
}

  from '../../service/search-product.service';
...
@Component({
  ...
  darkTheme(event: any) {
    ...
    if (document.body.style.cssText.includes('--text-color:#000')) {
      ...
      event.target.parentNode.parentNode.classList.remove("color-invert")
    } else {
      ...
      event.target.parentNode.parentNode.classList.add("color-invert")
    }
